...
Establish Relationships: Define relationships between tables (e.g., star schema) to enable accurate calculations across tables.
Proper relationships allow DAX measures to utilize context efficiently.

Create Calculated Columns: Use DAX expressions to create new columns at the row level within a table. 
Calculated columns are evaluated when the data is loaded or refreshed and are useful for static or row-level calculations.

2. Measure Creation

Define Measures: Measures are DAX formulas that perform calculations on your data, dynamically responding to filters and slicers in reports.
